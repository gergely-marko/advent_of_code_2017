...
sum_weight : Program -> Dict String Program -> Int
sum_weight program collection =
    case program.programs of
        [] ->
            program.weight
            ...
            programs
                |> List.filterMap
                    (\child_name ->
                        collection
                            |> Dict.get child_name
                            |> Maybe.map (\p -> sum_weight p collection)
                    )
                |> List.sum
                |> (+) program.weight


get_root : Dict String Program -> Maybe Program
get_root collection =
    let
        programs =
    ...
    in
        programs
            |> find
                (\program ->
                    programs
                        |> List.filter (\parent -> parent.name /= program.name)
                        |> find (\parent -> List.member program.name parent.programs)
                        |> Maybe.map (\p -> False)
                        |> Maybe.withDefault True
                )


get_programs : String -> Dict String Program
get_programs input =
    input
        |> String.lines
...
get_program line =
    ...
    in
        iterate_words words Name (Program "" 0 [])


iterate_words : List String -> Phase -> Program -> Program
iterate_words words phase program =
    case words of
        [] ->
            program
            ...
            case phase of
                ...
                Weight ->
                    w
                        ...
                        |> (\weight -> { program | weight = weight })
                        |> iterate_words tail Child
