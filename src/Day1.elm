module Day1 exposing (..)

import Html exposing (..)
import Array.Hamt as Array exposing (Array)


main : Html msg
main =
    input
        -- answer_1
        |> answer_2
        |> text


input : String
input =
    ""


test_input_1 : String
test_input_1 =
    "12131415"


answer_1 : String -> String
answer_1 input =
    input
        |> convert_1
        |> calculate_1
        |> (\result ->
                case result of
                    Err e ->
                        e

                    Ok r ->
                        toString r
           )


answer_2 : String -> String
answer_2 input =
    input
        |> convert_2
        |> calculate_2
        |> (\result ->
                case result of
                    Err e ->
                        e

                    Ok r ->
                        toString r
           )


convert_1 : String -> List Int
convert_1 input =
    input
        |> String.toList
        |> List.filterMap (String.fromChar >> String.toInt >> Result.toMaybe)
        |> (\list ->
                List.append list (list |> List.head |> Maybe.map (\x -> [ x ]) |> Maybe.withDefault [])
           )


convert_2 : String -> List Int
convert_2 input =
    input
        |> String.toList
        |> List.filterMap (String.fromChar >> String.toInt >> Result.toMaybe)


calculate_1 : List Int -> Result String Int
calculate_1 list =
    iterate list 0


iterate : List Int -> Int -> Result String Int
iterate list result =
    case list of
        [] ->
            Result.Err "Empty list"

        a :: b :: tail ->
            if a == b then
                iterate (b :: tail) (result + a)
            else
                iterate (b :: tail) result

        a :: [] ->
            Result.Ok result


calculate_2 : List Int -> Result String Int
calculate_2 list =
    let
        array =
            Array.fromList list

        delta =
            (Array.length array |> toFloat) / 2.0 |> round

        length =
            Array.length array

        indices =
            List.range 0 (length - 1)
                |> (\l -> Debug.log "indices" l)

        index i =
            if i < length then
                i
            else
                index (i - length)
    in
        indices
            |> List.filterMap (\i -> Array.get i array |> Maybe.map (\v -> Debug.log "item" ( i, v )))
            |> List.filterMap
                (\( i, v1 ) ->
                    i
                        + delta
                        |> index
                        |> (\i -> Array.get i array)
                        |> Maybe.andThen
                            (\v2 ->
                                if v1 == v2 then
                                    Just v1
                                else
                                    Nothing
                            )
                )
            |> List.sum
            |> Ok
